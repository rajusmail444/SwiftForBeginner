...
class NewsTableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        print("revealViewController is: \(revealViewController())")
        if revealViewController() != nil {
//            revealViewController().rearViewRevealWidth = 62
            menuButton.target = revealViewController()
            print("just before revealToggle")
    ...
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        //cell.targetForAction(#selector(self.navigateToRecipeApp(_:)), withSender: self)
        return cell
    }
    ...
    override func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        ...
        print("didSelectRowAtIndexPath:\(cell)")
        cell.tag = indexPath.row
        cell.targetForAction(#selector(NewsTableViewController.navigateToRecipeApp(_:)), withSender: self)
        //cell.targetForAction("navigateToRecipeApp:", withSender: self)
        print("didSelectRowAtIndexPath:\(cell)")
    }
    
    func navigateToRecipeApp(sender: AnyObject) {
        print("inside navigateToRecipeApp\(sender.tag)")
        if(sender.tag == 0) {
            ...
            let startUpViewController = UIStoryboard(name: "StartUp", bundle: nil).instantiateViewControllerWithIdentifier("StartUpViewController")
            presentViewController(startUpViewController, animated: true, completion: nil)
        }
    }
